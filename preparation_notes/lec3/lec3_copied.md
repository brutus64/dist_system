...
A popular approach to replication:  replicated state machine
  ...
  All replicas execute all operations
    If same start state,
      ...
      deterministic,
      then same end state.
 (Other approach to replication: state transfer)
...
Why would divergence be a disaster?
  ...
  Example: the 6.5840 homework submission server
...
A: There is no primary and no backup to take over. The system will be
down until a new VM has been started by the clustering service.
