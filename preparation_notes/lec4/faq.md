...
(Idea is that if its 1 server it comes 1 at a time, by circumstances, it is already linear. But if network drops a request but you actually performed it, resending the request may cause side effect if its not idempotent (e.g. sending deposits to a bank), so u need to account for that (like adding unique request ids to these requests), and for multiple machines/replications, if you call put on 1 machine and its result hasnt gone to its replicia yet, a get to the replica through some reverse proxy may result in a stale value.)
...
Strong Models:
Linearizability
Sequential Consistency: all operations appear in 1 global order, but it might not match real time (e.g. doesn't require if A finished before B started, everyone sees A first, global fictitious timeline)
Serializability: transactions (like DB transactions) where execution should be equivalent to some sequential ordering (e.g. T1: x =1; y=1; T2: x=2; y = 2, either T1 ran first then T2 or T2 ran then T1, no mix of both), ignores real-time delays and only cares about transaction boundary
Medium Models:
Causal Consistency: operator B depends on A, everyone must see A before B (unrelated writes can be seen in any order, but preserves happens-before which is if there's a dependence-relationship between operations it must occur in that order)
Timeline Consistency: each key/obj internally consistency over time but diff keys may diverge in diff order (x 1-> 2 -> 3, y: 10 -> 20, client can see x updates before y or y updates before x, in the keys itself the ordering is absolute but doesn't need to follow real-time for x and y together) basically per-key sequential consistency not cross-key
Weak Models:
Eventual Consistency: if no writes happens for long enough everyone should see the same value, NO GUARANTEE WHEN OR WHAT ORDER, just a very lax model
Fork Consistency: if server misbehaves, can fork clients to make them see diff histories but once forked, cannot rejoin (client A sees 1,2,3, B sees 1,4,5, diverged history, if A and B talk to each other, they're guaranteed to be able to detect the fork, server cannot merge together ever)
```
